<?php 
    ...
    $sql = 'SELECT * FROM test_subject WHERE `id`= '.$behavior_id;
    ...
    if ($result){
        if($record =  mysqli_fetch_array($result,MYSQLI_ASSOC)){
            ...
            $good = $record['student_behavior_good'];
            $bad = $record['student_behavior_bad'];
            $rate = $record['student_behavior_rate'];    
        }
    }
?>
<!DOCTYPE html>
...
<body style="background-color:#A6A6A6;">
    ...
    <div style="margin: 15% 25% 5% 25%;">
        <form action="" method="POST">
            ...
            <div class="container">
                    <div class="col-xs-4">
                        <label>Old Student Behavior Good</label>
                        <input type="text" name="oldbehaviorgood" value ="<?php echo $good;?>" class="form-control" >
                    </div>
                </div>
                <br>
                <div class="container">
                    <div class="col-xs-4">
                        <label>Old Student Behavior Bad</label>
                        <input type="text" name="oldbehaviorbad" value ="<?php echo $bad;?>" class="form-control" >
                    </div>
                </div>
                <br>
                <div class="container">
                    <div class="col-xs-4">
                        <label>Old Student Behavior Rate</label>
                        <input type="text" name="oldbehaviorrate" value ="<?php echo $rate;?>" class="form-control" >
                    </div>
                </div>
                <br>
            </div>
    ...
    <div style="margin: 15% 25% 25% 25%;">
        <form action="editbehavior.php?behavior_id=<?php echo $behavior_id?>" method="POST">
            <div style="background-color:#282D3D; color:#FFFFFF; padding:1rem; border-radius:20px;">
                <div class="container">
                    <div class="col-xs-4">
                        <label>Edit Student Behavior Good</label>
                        <input type="text" name="behaviorgood" class="form-control" placeholder="Enter Behavior Good" required>
                    </div>
                </div>
                <br>
                <div class="container">
                    <div class="col-xs-4">
                        <label>Edit Student Behavior Bad</label>
                        <input type="text" name="behaviorbad" class="form-control" placeholder="Enter Behavior Bad" required>
                    </div>
                </div>
                <br>
                <div class="container">
                    <div class="col-xs-4">
                    <label>Edit Student Behavior Rate</label>
                    <br>
...
<?php
    if(isset($_POST['update'])){
        $sql = 'UPDATE `test_subject` SET `student_behavior_good`="'.$_POST['behaviorgood'].'",`student_behavior_bad`="'.$_POST['behaviorbad'].'",`student_behavior_rate`="'.$_POST['rate'].'" WHERE `id` ='. $_GET["behavior_id"];
        $result = mysqli_query($connect,$sql);
        // if ($result){
        if($record =  mysqli_fetch_array($result,MYSQLI_ASSOC)){
            echo"<script>alert('Update success.');window.location.href = '/prowebtech/teacher/editbehavior.php';</script>";
        }
